...
async function main() {
    ...
    console.log("\nDeploy LockProxyReviewable on chain with Poly_Chain_Id:".cyan, polyId);
    
    if (config.LockProxyReviewable === undefined) {
        // deploy LockProxyReviewable
        console.log("\ndeploy LockProxyReviewable ......".cyan);
        lockproxy = await LockProxyReviewable.deploy();
        await lockproxy.deployed();
        console.log("LockProxyReviewable deployed to:".green, lockproxy.address.blue);
        config.LockProxyReviewable = lockproxy.address
        writeConfig(config)
    } else {
        console.log("\nLockProxyReviewable already deployed at".green, config.LockProxyReviewable.blue)
        lockproxy = await LockProxyReviewable.attach(config.LockProxyReviewable) 
    }
    ...
    if (alreadySetCCMP == ccmp) {
        ...
        // setup LockProxyReviewable
        console.log("\nsetup LockProxyReviewable ......".cyan);
